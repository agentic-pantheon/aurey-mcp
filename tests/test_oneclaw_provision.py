...
def test_provision_for_aurey_happy_path(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert result.zerion_secret_path is None
...
def test_provision_stores_zerion_secret(monkeypatch: pytest.MonkeyPatch) -> None:
    secret_puts: list[str] = []
    ...
    result = provision_for_aurey(
        "1ck_test",
        base_url="https://api.example",
        zerion_api_key="zerion-plain",
    )
    assert result.zerion_secret_path == "api-keys/zerion"
    assert any("/secrets/api-keys/zerion" in p for p in secret_puts)
