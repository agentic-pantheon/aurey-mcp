...
class AureySettings(BaseSettings):
    ...
    oneclaw_api_key_secret_source: str = Field(
        default="AUREY_ONECLAW_VAULT_API_KEY",
        description=(
            "Env var for the 1Claw **vault API key** (``ocv_…``) used for agent-token and "
            "vault reads—not the per-agent key from Platform bootstrap. "
            "Legacy alias: ``AUREY_ONECLAW_BOOTSTRAP_API_KEY``."
        ),
    )
    ...
    alchemy_api_secret_path: str | None = Field(
        default=None,
        description=(
            "Preferred: 1Claw vault path for the Alchemy API key (e.g. ``api-keys/alchemy``). "
            "Resolved via the vault API key; do not put Alchemy in MCP ``env`` when using this."
        ),
        validation_alias=AliasChoices("AUREY_ALCHEMY_API_SECRET_PATH"),
    )
    ...
    route_builder_api_key: str | None = Field(
        default=None,
        description="Optional bearer token for the Aurey route-builder.",
        validation_alias=AliasChoices("AUREY_ROUTE_BUILDER_API_KEY"),
    )
    dashboard_enabled: bool = Field(
        default=False,
        description="Serve local portfolio dashboard on localhost when true.",
        validation_alias=AliasChoices("AUREY_DASHBOARD_ENABLED"),
    )
    dashboard_host: str = Field(
        default="127.0.0.1",
        validation_alias=AliasChoices("AUREY_DASHBOARD_HOST"),
    )
    dashboard_port: int = Field(
        default=8765,
        ge=1,
        le=65535,
        validation_alias=AliasChoices("AUREY_DASHBOARD_PORT"),
    )
    dashboard_auth_token: str | None = Field(
        default=None,
    ...
    evm_signing_mode: EvmSigningMode = Field(
        ...
        description=(
            "How EVM transactions are signed. aurey-wallet-mcp requires ``oneclaw_intents`` "
            "(agent wallet from 1Claw signing-keys)."
        ),
    )
    ...
    deep_agent_wallet_address: str | None = Field(
        default=None,
        description=(
            "Optional EVM address override. When unset, the plugin uses the Ethereum address "
            "from 1Claw GET signing-keys for ``oneclaw_agent_id`` (Intents onboarding)."
        ),
    )
    ...
    def resolve_oneclaw_bootstrap_api_key(self) -> str:
        ...
        Tries ``oneclaw_api_key_secret_source`` first, then legacy env names.
        """

        primary = self.oneclaw_api_key_secret_source.strip()
        candidates: list[str] = []
        if primary:
            candidates.append(primary)
        for legacy in ("AUREY_ONECLAW_VAULT_API_KEY", "AUREY_ONECLAW_BOOTSTRAP_API_KEY"):
            if legacy not in candidates:
                candidates.append(legacy)
        last_key = primary or "AUREY_ONECLAW_VAULT_API_KEY"
