        ...
        try:
            if rb_base:
                rb_headers = {
                    ...
                    "User-Agent": _LIFI_HTTP_USER_AGENT,
                }
                rb_key = (runtime.settings.route_builder_api_key or "").strip()
                if rb_key:
                    rb_headers["Authorization"] = f"Bearer {rb_key}"
                rb_payload = runtime.http.request_json(
                    method="POST",
                    url=f"{rb_base}/v1/quote",
                    headers=rb_headers,
                    json_body={"query": q},
                )
                payload = (
                    rb_payload.get("quote")
                    if isinstance(rb_payload, dict) and isinstance(rb_payload.get("quote"), dict)
                    else rb_payload
                )
            else:
