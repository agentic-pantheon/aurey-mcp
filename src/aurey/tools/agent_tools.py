...
class GetAgentWalletAddressesArgs(BaseModel):
    refresh: bool = Field(
        default=False,
        description="When true, re-fetch signing-keys from 1Claw before returning addresses.",
    )
...
def build_aurey_subgraph_tools(runtime: AureyRuntime) -> list[BaseTool]:
    ...
    @tool(args_schema=GetAgentWalletAddressesArgs)
    def get_agent_wallet_addresses(refresh: bool = False) -> dict[str, Any]:
        """Return Ethereum/Solana addresses for ``AUREY_ONECLAW_AGENT_ID`` from 1Claw signing-keys.

        Call this before ``swap_prepare`` or transfers when you need ``from_address``. Do not guess ``0x``."""
        if refresh:
            eth, sol = fetch_agent_wallet_addresses_from_oneclaw(runtime)
            status = agent_wallet_status(runtime)
            if eth:
                status = {**status, "ethereum": eth}
            if sol:
                status = {**status, "solana": sol}
            return {"ok": True, "result": status}
        return {"ok": True, "result": agent_wallet_status(runtime)}
    ...
    tools.append(get_agent_wallet_addresses)
